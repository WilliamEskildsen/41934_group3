...
def writeCustomHTML(model):
    ...
    # ---- ADD WINDOWS CUSTOM ENTITIES - custom made by group 3
    ...
    custom+= classifyWindowTotalArea(windows)
...
def classifyFloors(floors,site_elev,windows):
    ...
    for floor in floors:
        ...
        for window in windows:
            ...
            if WindowLevel == floor.Name:    
                window_entities+=7*"\t"+"<window- level=\""+WindowLevel+"\" name='{}' area='{}'  </window->\n".format(window.Name,window.OverallWidth*window.OverallHeight)
                WindowAreaByFloor += window.OverallHeight*window.OverallWidth
        
        floor_entities+=6*"\t"+"<floor- class=\""+type+"\" WindowAreaByFloor=\""+str(round(WindowAreaByFloor,3))+"\" name='{}'  level='{}' elev=\"{}\" >{}<span class=\"floor_stats\">{}</span> </floor->\n".format(floor.Name, level, floor.Elevation,floor.Name, round(float(floor.Elevation),3))
    ...
    return floor_entities + window_entities
...
def classifyWindowTotalArea(windows):
    ...
    AreaOfWindows +=8*"\t"+ "<AreaOfWindows- TotalArea=\""+str(round(TotalWindowArea,3))+"\"\n"
